    ...
    with col2:
        with st.expander("📄 Exportar PDF", expanded=True):
            modo_pdf = st.radio(
                ...
                ["Cidade selecionada", "Bairro selecionado", "Múltiplas cidades"],
                horizontal=True,
            )
            ...
            elif modo_pdf == "Bairro selecionado":
                cidades_pdf = sorted(df_f["CIDADE"].dropna().unique().tolist())
                cidade_pdf  = st.selectbox("Cidade", cidades_pdf, key="cid_bairro_pdf")
                bairros_pdf = sorted(df_f[df_f["CIDADE"] == cidade_pdf]["BAIRRO"].replace("", pd.NA).dropna().unique())
                bairro_pdf  = st.selectbox("Bairro", bairros_pdf if bairros_pdf else ["(sem bairro)"], key="bairro_pdf")

                if st.button("⬇️ Gerar PDF — bairro", use_container_width=True):
                    df_c = df_f[df_f["CIDADE"] == cidade_pdf]
                    with st.spinner(f"Gerando PDF de {bairro_pdf} / {cidade_pdf}…"):
                        pdf = gerar_pdf_bairro(df_c, cidade_pdf, bairro_pdf)
                    if pdf:
                        st.download_button(
                            f"📥 Baixar PDF — {bairro_pdf}",
                            data=pdf,
                            file_name=f"hapvida_{cidade_pdf}_{bairro_pdf}.pdf".replace(" ","_"),
                            mime="application/pdf",
                            use_container_width=True,
                        )
                    else:
