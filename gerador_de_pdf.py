...
def gerar_pdf_bairros(df: pd.DataFrame, cidade: str, bairros: List[str]) -> Optional[bytes]:
    """
    Gera um único PDF contendo apenas os bairros selecionados de uma cidade.
    Reaproveita o agrupamento por bairro já existente em _gerar_pdf
    (cada bairro continua começando em uma nova página).
    """
    try:
        df_b = df[df["BAIRRO"].isin(bairros)].copy()
        if df_b.empty:
            return None
        return _gerar_pdf(df_b, cidade)
    except Exception:
        import traceback; traceback.print_exc(); return None
